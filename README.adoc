...
To test _Ontology_ you have two possible options:
...
image::./doc/images/diagram-ontology.png[Ontology Diagram]

You can access link:./doc/user.adoc[the full user documentation] in this repository.
...
1. Ontology uses PostgreSQL for its database.
For development or local testing, the easiest way is to start a PostgreSQL instance using Docker.
+
[#docker-db, reftext=Docker command]
[source,sh]
----
docker run -p 5437:5432 --name ontology-postgres \
...
+
Get the last published `ontology-application-{$version}.jar` from https://github.com/orgs/ObeoNetwork/packages?repo_name=ontology[Ontology github package]
+
[source,sh]
----
java -jar ontology-application-{$version}.jar \
          --spring.datasource.url=jdbc:postgresql://localhost:5437/ontology-db \
          --spring.datasource.username=dbuser \
          --spring.datasource.password=dbpwd
----
+
Alternatively, if you already have `backend/ontology-application/target/ontology-application-{$version}.jar`, you can build and run a Docker image for the application:
+
[source,sh]
----
docker build \
          --build-arg ONTOLOGY_VERSION={$version} \
          -t ontology-application:{$version} \
          backend/ontology-application

docker run --rm -p 8080:8080 --name ontology-application \
          ontology-application:{$version} \
          --spring.datasource.url=jdbc:postgresql://host.docker.internal:5437/ontology-db \
          --spring.datasource.username=dbuser \
          --spring.datasource.password=dbpwd
----
+
If you prefer to start both PostgreSQL and Ontology together, use the Docker Compose file at the repository root:
+
[source,sh]
----
...
+
WARNING: The initial version of Ontology has some known issues with Firefox.
It is recommended to use a Chrome-based browser until these are fixed.
+
NOTE: Do not forget to stop the PostgreSQL container once you are done: `docker stop ontology-postgres`.
...
To remove this limitation, apply the https://learn.microsoft.com/en-us/windows/win32/fileio/maximum-file-path-limitation?tabs=powershell[this procedure] with this command line in PowerShell (`New-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem" -Name "LongPathsEnabled" -Value 1 -PropertyType DWORD -Force`), then active the longpath option in Git in a command line with administrator rights (`git config --system core.longpaths true`).

[#github-token]
=== GitHub Access Token

The backend of _Ontology_ depends on https://github.com/eclipse-sirius/sirius-emf-json[`sirius-emf-json`] and https://github.com/eclipse-sirius/sirius-web[`sirius-web`], which is published as Maven artifacts in _GitHub Packages_.
To build `ontology` locally, you need a _GitHub Access Token_ so that Maven can download the artifacts.
...
[#build-steps]
=== Build Ontology application

Build steps:

. Clone https://github.com/ObeoNetwork/ontology[Ontology repository]

. Build the frontend ([.small]#from the `frontend` subfolder of Ontology main location#):
+
[source,sh]
----
npm ci
...
NOTE: In case of npm ERR!
Lifecycle script `format-lint` failed with error, try to run from the frontend subfolder the following command:
`npm run format`

. Install the frontend artifacts as static resource to be served by the backend.
From the root directory of the repository:
+
[source,sh]
----
mkdir -p backend/ontology-frontend/src/main/resources/static
cp -R frontend/ontology-web/dist/* backend/ontology-frontend/src/main/resources/static
----

. Build the backend ([.small]#from the `backend` subfolder of Ontology main location#):
+
[source,sh]
----
mvn clean verify
----
+
The result is a ready-to-run, Spring Boot "fat JAR" in backend/ontology-application/target/ontology-application-<VERSION>.jar.
...
* Start the server with `ontologyWeb.java` class.
...
Except when explicitly mentioned otherwise (e.g. for some resources likes images), the license is Eclipse Public License* v 2.0.
