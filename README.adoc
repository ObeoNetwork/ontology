...
Ontology application is an application based on Sirius-Web and Obeo Enterprise for Sirius to manage ontologies.
...
+
Get the `ontology-application-{$version}.jar` from the last successful build on https://github.com/ObeoNetwork/ontology[Ontology github]
+
[source,sh]
----
java -jar ontology-application-{$version}.jar \
          --spring.datasource.url=jdbc:postgresql://localhost:5437/ontology-web-db \
...
[source]
----
backend/ontology-resources/checkstyle/CheckstyleConfiguration.xml
----
...
[source]
----
backend/ontology-resources/editor
----
