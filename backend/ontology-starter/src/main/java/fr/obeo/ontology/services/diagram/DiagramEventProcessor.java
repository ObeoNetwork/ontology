 ...
 *******************************************************************************/
package fr.obeo.ontology.services.diagram;
...
import org.eclipse.sirius.components.collaborative.diagrams.DiagramEventFlux;
...
/**
 * Reacts to input that target a specific diagram, and {@link #getOutputEvents(IInput)}  publishes} updated versions of the diagram to interested subscribers.
 *
 * @author sbegaudeau
 * @author pcdavid
 */
public class DiagramEventProcessor implements IDiagramEventProcessor {
    ...
    public DiagramEventProcessor(DiagramEventProcessorParameters parameters) {
        ...
        Diagram diagramAfterSave = this.representationSearchService.findById(this.editingContext, diagram.getId(), Diagram.class).orElse(diagram);

        this.diagramContext.update(diagramAfterSave);
        this.diagramEventFlux = new DiagramEventFlux(diagramAfterSave);
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IRepresentationInput representationInput) {
        if (representationInput instanceof LayoutDiagramInput layoutDiagramInput) {
            if (LayoutDiagramInput.CAUSE_LAYOUT.equals(layoutDiagramInput.cause()) || layoutDiagramInput.id().equals(this.currentRevisionId)) {
                ...
                Diagram diagramAfterSave = this.representationSearchService.findById(this.editingContext, laidOutDiagram.getId(), Diagram.class).orElse(laidOutDiagram);
                this.diagramContext.reset();
                this.diagramContext.update(diagramAfterSave);
                this.diagramEventFlux.diagramRefreshed(layoutDiagramInput.id(), diagramAfterSave, DiagramRefreshedEventPayload.CAUSE_LAYOUT, null);
    ...
    public void refresh(ChangeDescription changeDescription) {
        if (this.shouldRefresh(changeDescription)) {
            ...
            Diagram diagramAfterSave = this.representationSearchService.findById(this.editingContext, refreshedDiagram.getId(), Diagram.class).orElse(refreshedDiagram);

            if (diagramAfterSave != null) {
                this.logger.trace("Diagram refreshed: {}", diagramAfterSave.getId());
            }
            ...
            this.diagramEventFlux.diagramRefreshed(changeDescription.getInput().id(), diagramAfterSave, DiagramRefreshedEventPayload.CAUSE_REFRESH, referencePosition);
    ...
    /**
     * A diagram is refresh if there is a semantic change or if there is a diagram layout change coming from this very diagram (not other diagrams).
     *
